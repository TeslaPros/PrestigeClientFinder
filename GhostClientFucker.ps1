...
# Known framework dictionary mapping strings to actual client tags
$clientFrameworks = @{
    ...
    "doomsdayclient" = "Doomsday PvP Client"; "DoomsdayClient" = "Doomsday PvP Client"; "doomsday.jar" = "Doomsday Engine"
...
# ═════════════════════════════════════════════════════════════════════════════════════════
#                      ✨ HIGHLY DETAILED ENG RECON SUMMARY REPORT ✨
# ═════════════════════════════════════════════════════════════════════════════════════════
Clear-Host
Write-Host $Banner -ForegroundColor Cyan
Write-Host "`n"
...
Write-Row "═" 90 Cyan
Write-Host "                      TESLAPRO FORENSIC SCANNER DETAILED SUMMARY REPORT                      " -ForegroundColor White
Write-Row "═" 90 Cyan
Write-Host ""
Write-Host "  [+] TARGET SCAN PATH : " -NoNewline -ForegroundColor Gray; Write-Host "$modsPath" -ForegroundColor White
Write-Host "  [+] TOTAL FILES SEEN : " -NoNewline -ForegroundColor Gray; Write-Host "$totalFiles JAR files checked" -ForegroundColor White
Write-Host "  [+] SECURITY STATUS  : " -NoNewline -ForegroundColor Gray
if ($flaggedMods.Count -gt 0) {
    Write-Host "COMPROMISED - MALICIOUS OR UNLAWFUL UTILITIES FOUND" -ForegroundColor Red
} else {
    Write-Host "SECURE - ALL INVESTIGATED MODULES MATCH SAFE STANDARDS" -ForegroundColor Green
}
Write-Host ""

Write-Row "─" 90 DarkGray
Write-Host " 🛑 DETECTED FRAUDULENT MODULES & CLIENT ASSEMBLIES ($($flaggedMods.Count) Flags Found)" -ForegroundColor Red
Write-Row "─" 90 DarkGray
Write-Host ""

if ($flaggedMods.Count -eq 0) {
    Write-Host "  📋 No malicious components or unauthorized ghost clients were flagged inside this directory." -ForegroundColor Green
    Write-Host ""
} else {
    foreach ($mod in $flaggedMods) {
        Write-Host "  [💥] MOD FILENAME : " -NoNewline -ForegroundColor White
        Write-Host "$($mod.File)" -ForegroundColor Red
        
        Write-Host "       ├── Identified Client Base : " -NoNewline -ForegroundColor Gray
        Write-Host "$($mod.Client)" -ForegroundColor Yellow
        
        Write-Host "       ├── Network Fetch Origin   : " -NoNewline -ForegroundColor Gray
        Write-Host "$($mod.Source)" -ForegroundColor DarkYellow
        
        Write-Host "       └── Extraction Indicators  : " -NoNewline -ForegroundColor Gray
...
Write-Row "─" 90 DarkGray
Write-Host " ✅ TRUSTED AND VERIFIED PACKAGES ($($cleanMods.Count) Files Passed)" -ForegroundColor Green
Write-Row "─" 90 DarkGray
Write-Host ""

if ($cleanMods.Count -eq 0) {
    Write-Host "  [!] Zero modules returned verified tags or clean status benchmarks." -ForegroundColor Orange
    Write-Host ""
} else {
...
Write-Host "  ────────────────────────────────"
Write-Host "  • Total Archive Population  : " -NoNewline -ForegroundColor Gray; Write-Host "$totalFiles" -ForegroundColor White
Write-Host "  • Suspicious/Flagged Files  : " -NoNewline -ForegroundColor Gray; Write-Host "$($flaggedMods.Count)" -ForegroundColor Red
Write-Host "  • Legitimate/Clean Files    : " -NoNewline -ForegroundColor Gray; Write-Host "$($cleanMods.Count)" -ForegroundColor Green
Write-Host ""
Write-Row "═" 90 Cyan
Write-Host ""
